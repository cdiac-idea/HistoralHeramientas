...
error_reporting(0);
	...
	function getEstacionClima($visible){
		...
		$sql = "SELECT DISTINCT estacion_sk as id, estacion as nombre, municipio as municipio 
				FROM station_dim s
				WHERE tipologia!='Calidad del aire'";
		if ($visible=='true') {
			$sql .=" AND s.visible='".$visible."'";
		}
		$sql .= "ORDER BY nombre";
	...
	function getEstacionOrderId($visible){
		...
		$sql .= " GROUP BY 1, 2 ORDER BY municipio, tipologia ";
		return $objDatos->executeQuery($sql);
	}
	...
	function getEstacionVarAire($variable){
		...
		$sql = "SELECT DISTINCT station_dim.estacion_sk as id, estacion as nombre, municipio as municipio  
				...
				WHERE station_dim.tipologia='Calidad del aire' AND fact_aire.$variable IS NOT NULL 
	...
	****************************************************************************************/
	...
	function extraTempsFact($idEstacion, $idAno, $mes, $dia){
		...
		$sql = "SELECT dia AS dia, 
				...
				FROM fact_table, date_dim
				WHERE fact_table.estacion_sk=".$idEstacion." AND fact_table.fecha_sk=date_dim.fecha_sk 
					AND date_dim.año=".$idAno." AND date_dim.mes=".$mes." AND date_dim.dia=".$dia." group by 1";
		return $objDatos->executeQuery($sql);
	}
		...
		//obtiene el dia mas alto que existe se obtienen de la fact_table y date_dim
			...
			if ($mes==1) {
				for ($dia=1; $dia < $diaLimite; $dia++){
					...
					$mes=12;
					$diaTemporal = $dia;
					$limtDiaTemporal = $limtDia;
					$limtDia=31;
					$dia = $limtDia - $diaLimite + $diaTemporal +1;
					$valorA25 = calculoA25($idEstacion, $idAno, $mes, $dia, $limtDia);
					$idAno++;
					$mes=1;
					$diaFin = $diaTemporal;
					$limtDia = $limtDiaTemporal;
					$dia = $diaTemporal;
					$valorA25 = $valorA25 + calculoA25($idEstacion, $idAno, $mes, 1, $diaFin);
					//echo "<br> valorA25 ".$valorA25;
					escribeA25($idEstacion, $idAno, $mes, $diaFin, $valorA25);
				}
			}
			for ($dia=1; $dia <= $limtDia; $dia++) { 
				$diaFin = $dia + $diaLimite - 1;
				if ($diaFin > $limtDia ){
					$valorA25 = calculoA25($idEstacion, $idAno, $mes, $dia, $limtDia);
					$diaFin = $limtDia - $dia; //Lo que falta el mes
					$diaFin = $diaLimite - $diaFin - 1;
					$mes = $mes + 1;
					$valorA25 = $valorA25 + calculoA25($idEstacion, $idAno, $mes, 1, $diaFin);
					escribeA25($idEstacion, $idAno, $mes, $diaFin, $valorA25);
					$mes = $mes-1;
				}else{
					$valorA25 = $valorA25+calculoA25($idEstacion, $idAno, $mes, $dia, $diaFin);
					escribeA25($idEstacion, $idAno, $mes, $diaFin, $valorA25);
				}
			}
		}
	...
	function generarDvvMes($idEstacion, $idAno, $variable){
		...
		for ($mes=1; $mes <= $limtMes; $mes++) { 
			...
			$sql = "SELECT SUM(pre_n_dia_diurno) AS nd, SUM(pre_nne_dia_diurno) AS nned, 
							SUM(pre_ne_dia_diurno) AS ned, SUM(pre_ene_dia_diurno) AS ened, 
							SUM(pre_e_dia_diurno) AS ed, SUM(pre_ese_dia_diurno) AS esed, 
							SUM(pre_se_dia_diurno) AS sed, SUM(pre_sse_dia_diurno) AS ssed, 
							SUM(pre_s_dia_diurno) AS sd, SUM(pre_sso_dia_diurno) AS ssod, 
							SUM(pre_so_dia_diurno) AS sod, SUM(pre_oso_dia_diurno) AS osod, 
							SUM(pre_o_dia_diurno) AS od, SUM(pre_ono_dia_diurno) AS onod, 
							SUM(pre_no_dia_diurno) AS nod, SUM(pre_nno_dia_diurno) AS nnod, 
							SUM(pre_n_dia_nocturno) AS nn, SUM(pre_nne_dia_nocturno) AS nnen, 
							SUM(pre_ne_dia_nocturno) AS nen, SUM(pre_ene_dia_nocturno) AS enen, 
							SUM(pre_e_dia_nocturno) AS en, SUM(pre_ese_dia_nocturno) AS esen, 
							SUM(pre_se_dia_nocturno) AS sen, SUM(pre_sse_dia_nocturno) AS ssen, 
							SUM(pre_s_dia_nocturno) AS sn, SUM(pre_sso_dia_nocturno) AS sson, 
							SUM(pre_so_dia_nocturno) AS son, SUM(pre_oso_dia_nocturno) AS oson, 
							SUM(pre_o_dia_nocturno) AS onoc, SUM(pre_ono_dia_nocturno) AS onon, 
							SUM(pre_no_dia_nocturno) AS non, SUM(pre_nno_dia_nocturno) AS nnon
							FROM indicador
							WHERE estacion=".$idEstacion." AND anio=".$idAno." AND mes=".$mes;
			$dato = $objDatos->executeQuery($sql);
			$dato = $dato[0];
			$sql = "UPDATE indicador SET pre_n_dia_diurno=".$dato['nd'].", pre_nne_dia_diurno=".
					$dato['nned'].", pre_ne_dia_diurno=".$dato['ned'].",  pre_ene_dia_diurno=".
					$dato['ened'].", pre_e_dia_diurno=".$dato['ed'].", pre_ese_dia_diurno=".
					$dato['esed'].", pre_se_dia_diurno=".$dato['sed'].", pre_sse_dia_diurno=".
					$dato['ssed'].", pre_s_dia_diurno=".$dato['sd'].",pre_sso_dia_diurno=".
					$dato['ssod'].", pre_so_dia_diurno=".$dato['sod'].", pre_oso_dia_diurno=".
					$dato['osod'].", pre_o_dia_diurno=".$dato['od'].", pre_ono_dia_diurno=".
					$dato['onod'].", pre_no_dia_diurno=".$dato['nod'].", pre_nno_dia_diurno=".
					$dato['nnod'].", pre_n_dia_nocturno=".$dato['nn'].", pre_nne_dia_nocturno=".
					$dato['nnen'].", pre_ne_dia_nocturno=".$dato['nen'].", pre_ene_dia_nocturno=".
					$dato['enen'].", pre_e_dia_nocturno=".$dato['en'].", pre_ese_dia_nocturno=".
					$dato['esen'].", pre_se_dia_nocturno=".$dato['sen'].", pre_sse_dia_nocturno=".
					$dato['ssen'].", pre_s_dia_nocturno=".$dato['sn'].", pre_sso_dia_nocturno=".
					$dato['sson'].", pre_so_dia_nocturno=".$dato['son'].", pre_oso_dia_nocturno=".
					$dato['oson'].", pre_o_dia_nocturno=".$dato['onoc'].", pre_ono_dia_nocturno=".
					$dato['onon'].", pre_no_dia_nocturno=".$dato['non'].", pre_nno_dia_nocturno=".
					$dato['nnon']." WHERE estacion=".$idEstacion." AND anio=".$idAno." AND mes=".$mes." AND dia=0";
			if ($dato['nnon']!=null) {
				$objDatos->operacionesCrud($sql);
				$division = array_sum($dato);
				$frec = array();
				$nombre = array( 0=>'nd', 1=>'nned', 2=>'ned', 3=>'ened', 4=>'ed', 5=>'esed', 6=>'sed', 
									7=>'ssed', 8=>'sd', 9=>'ssod', 10=>'sod',  11=>'osod', 12=>'od', 
									13 =>'onod', 14=>'nod',  15=>'nnod', 16=>'nn',  17=>'nnen', 18=>'nen', 
									19 =>'enen', 20=>'en',  21=>'esen', 22=>'sen',  23=>'ssen', 24=>'sn',  
									25=>'sson', 26=>'son',  27=>'oson', 28=>'onoc',  29=>'onon', 30=>'non',  
									31=>'nnon');
				for ($i=0; $i < 32; $i++) { 
					$frec[$i]=($dato[$nombre[$i]]/$division)*100;
				}
				$sql = "UPDATE indicador SET n_dia_diurno=".$frec[0].", nne_dia_diurno=".
						$frec[1].", ne_dia_diurno=".$frec[2].",  ene_dia_diurno=".
						$frec[3].", e_dia_diurno=".$frec[4].", ese_dia_diurno=".
						$frec[5].", se_dia_diurno=".$frec[6].", sse_dia_diurno=".
						$frec[7].", s_dia_diurno=".$frec[8].",sso_dia_diurno=".
						$frec[9].", so_dia_diurno=".$frec[10].", oso_dia_diurno=".
						$frec[11].", o_dia_diurno=".$frec[12].", ono_dia_diurno=".
						$frec[13].", no_dia_diurno=".$frec[14].", nno_dia_diurno=".
						$frec[15].", n_dia_nocturno=".$frec[16].", nne_dia_nocturno=".
						$frec[17].", ne_dia_nocturno=".$frec[18].", ene_dia_nocturno=".
						$frec[19].", e_dia_nocturno=".$frec[20].", ese_dia_nocturno=".
						$frec[21].", se_dia_nocturno=".$frec[22].", sse_dia_nocturno=".
						$frec[23].", s_dia_nocturno=".$frec[24].", sso_dia_nocturno=".
						$frec[25].", so_dia_nocturno=".$frec[26].", oso_dia_nocturno=".
						$frec[27].", o_dia_nocturno=".$frec[28].", ono_dia_nocturno=".
						$frec[29].", no_dia_nocturno=".$frec[30].", nno_dia_nocturno=".
						$frec[31].
						" WHERE estacion=".$idEstacion." AND anio=".$idAno." AND mes=".$mes." AND dia=0";
				$objDatos->operacionesCrud($sql);
			}
	...
	function generarDvvAnual($idEstacion, $idAno, $variable){
		...
		$sql = "UPDATE indicador SET pre_n_dia_diurno=".$dato['nd'].", pre_nne_dia_diurno=".
				...
				$dato['oson'].", pre_o_dia_nocturno=".$dato['onoc'].", pre_ono_dia_nocturno=".
				$dato['onon'].", pre_no_dia_nocturno=".$dato['non'].", pre_nno_dia_nocturno=".
				$dato['nnon']." WHERE estacion=".$idEstacion." AND anio=".$idAno." AND mes=0 AND dia=0";
		if ($dato['nnon']!=null) {
			$objDatos->operacionesCrud($sql);
			$division = array_sum($dato);
			$frec = array();
		...
		for ($i=1; $i < $tam; $i = $i+2) { 
			$sql = "SELECT COUNT(direccion_viento)
					...
					WHERE fact_table.direccion_viento>=".$rango[$i-1]." 
							...
							AND fact_table.estacion_sk=".$idEstacion."  
							AND date_dim.año=".$idAno." 
							AND date_dim.mes=".$mes." AND date_dim.dia=".$dia."
							AND ((fact_table.tiempo_sk>=".$iniRang1." 
