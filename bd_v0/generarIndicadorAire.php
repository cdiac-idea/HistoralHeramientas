<?php   
    ...
    if ($_POST) {
		...
		if ($idVariable == 173) {
			#$titulo1 .= "<td>Concentración</td><td>ICA PM10</td><td>Clasificación</td>";
			$titulo1 .= "<td>ICA PM10</td><td>Clasificación</td>"; //<td>Gráfica</td>"; //
			$archivoPlano .= $separador."ICA PM10".$separador."Clasificación";
			#$sql .= ", f.pm10, i.ica_pm10, i.calificacion_pm10";
		...
		if ($idVariable == 174) {
			$titulo1 .= "<td>ICA PM2,5</td><td>Clasificación</td>"; //<td>Gráfica</td>";
		...
		if ($idVariable == 175) {
			$titulo1 .= "<td>Concentración<br>(ppm)</td><td>ICA CO</td><td>Clasificación</td>
							<td>Confianza (%)</td><td>Desviación <br>Estándar</td>"; //<td>Gráfica</td>";
			$archivoPlano .= $separador."Concentración".$separador."ICA CO".$separador."Clasificación"
							.$separador."Confianza (%)".$separador."Desviación Estándar";
			$sql .= ", i.concentracion_co_8h, i.ica_co_8h, i.calificacion_co_8h, i.confianza_co_8h, i.desviacion_co_8h";
			//verificar si ya existe el dato
			$validar = validarIndAire($idEstacion, $idVariable, $menor, $mayor);
			if ($validar) {
		...
		if ($idVariable == 176) {
			$titulo1 .= "<td>Concentración<br>(ppm)</td><td>ICA O3</td><td>Clasificación</td><td>Confianza (%)</td>
						<td>Desviación <br>Estándar</td>"; //<td>Gráfica</td>";
			$archivoPlano .= $separador."Concentración".$separador."ICA O3".$separador."Clasificación"
							.$separador."Confianza (%)".$separador."Desviación Estándar";
			$sql .= ", i.concentracion_o3_8h, i.ica_o3_8h, i.calificacion_o3_8h, i.confianza_o3_8h, i.desviacion_o3_8h";
			//verificar si ya existe el dato
			$validar = validarIndAire($idEstacion, $idVariable, $menor, $mayor);
			if ($validar) {
		...
		$titulo1 .= "</tr>";
		#$sql .= " FROM fact_aire f, indicador_aire i, date_dim d WHERE i.estacion=".$idEstacion." AND i.anio>=".$menor." AND i.anio<=".$mayor." AND f.estacion_sk=".$idEstacion." AND d.fecha_sk=f.fecha_sk AND d.año>=".$menor." AND d.año<=".$mayor." ORDER BY 1, 2, 3";
