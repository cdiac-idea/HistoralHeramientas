...
//$estacionElegida = 'est_rioclaro';// tabla en la cual se encuentral los datos



// se crea la tabla temporal para el proceso de filtrado
//crear_central();
...
$consulta_bases = "SELECT * from basedatos where activa = 1"; #!! QUITAR EL OFFSET PARA TOMAR TODAS LAS BASES DE DATOS, acá solo toma la primera base para efectos de pruebas
...
while($f_consulta_bases = pg_fetch_array($e_consulta_bases)){
	...
	$con_tablas = "SELECT * from tablas where id_basedatos = $f_consulta_bases[id_basedatos] order by id_tabla"; //OBTENGO La primera por cuestion de pruebas
	$e_con_tablas = pg_query($con_tablas);

	while ($f_con_tablas = pg_fetch_array($e_con_tablas)) {
		crear_central();

		$tabla = $f_con_tablas['nombre_tabla'];

		$ultima_fecha_filtrada = "SELECT dato_actual from tablas where nombre_tabla = '$tabla'";  #|
		...
		$ultima_hora_filtrada = "SELECT hora_actual from tablas where nombre_tabla = '$tabla'";   #|
		...
		########################################################################################
		obtener_datos($tabla,$dbr,$fecha,$hora,$Acumulador);
		########################################################################################
...
function myFunction() {
	...
	<?php
	/*
	$consultoNumeroDatos = "SELECT count (estacion_sk) as cantidad from fact_table where estacion_sk = $parcial";
...
// se elabora la cadena de columnas a consultar
function obtener_datos($tabla,$dbr,$fecha,$hora,$Acumulador){
			...
			$Acumulador = $Acumulador + $n_consuta;
...
############################# senda parte del insert ##################
			...
			$Acumulador = $Acumulador + $n_consuta2;
