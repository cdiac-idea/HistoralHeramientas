...
require_once("../conexion/conexiondwh.php");
 ...
 function  calculo_entrantes(){	
 		...
 		$col= "SELECT nombre from variable where tipologia = 'Aire'";
 		$e_col = pg_query($col);
 		...
 		while ($f_col = pg_fetch_array($e_col)) {
			...
			$calculo_entrantes ="SELECT estacion_sk,fecha_sk,count($columna) as cantidad from central_aire where $columna != '-' and $columna != '' or $columna is null GROUP BY  estacion_sk,fecha_sk";
        	...
        	while ($f_calculo_entrantes = pg_fetch_array($e_calculo_entrantes)) {
        		...
        		$existe = "SELECT estacion_sk,fecha_sk from confiabilidad_aire where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk";
				...
				if ($n_exite == 0){
						...
						$insert = "INSERT INTO confiabilidad_aire (estacion_sk,fecha_sk,total_entrante_".$columna.",total_buenos_".$columna.") values ($estacion_sk,$fecha_sk,$cantidad_entrantes,$cantidad_entrantes)";
					...
					$existe_dato = "SELECT total_entrante_".$columna." from confiabilidad_aire where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk";
					...
					if ($n_existe_dato == 0) {
						...
						$update = "UPDATE confiabilidad_aire set total_entrante_".$columna." = $entrantes where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk";
						...
						$update = "UPDATE confiabilidad_aire set $variable2 = $suma where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk";
						$e_update = pg_query($update);
					...
					$existe_dato = "SELECT total_buenos_".$columna." from confiabilidad_aire where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk";
        			$e_existe_dato = pg_query($existe_dato);
					$f_existe_dato = pg_fetch_array($e_existe_dato);
					$n_existe_dato = pg_num_rows($e_existe_dato);
					...
					if ($n_existe_dato == 0) {
						...
						$entrantes = (double)$cantidad_entrantes;
						$update = "UPDATE confiabilidad_aire set total_buenos_".$columna." = $entrantes where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk";
						$e_update = pg_query($update);
					}
					else{
						...
						$entrantes = (double)$cantidad_entrantes;
						$suma = $cantidad_antes + $entrantes;
						$update = "UPDATE confiabilidad_aire set $variable = $suma where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk";
						$e_update = pg_query($update);
...
function insertar_errores(){
	...
	$update_correcciones = "UPDATE confiabilidad_aire set total_buenos_".$variable." = total_buenos_".$variable." - $cantidad where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk"; 
...
$obtener = "SELECT estacion_sk,fecha_sk,$variable_entrantes,$variable_buenos from confiabilidad_aire where soporte_".$variable." is null";
...
while ($f_obtener = pg_fetch_array($e_obtener)) {
	...
	$update_tabla_conf = "UPDATE confiabilidad_aire set soporte_".$variable." = $soporte, confianza_".$variable." = $confianza where estacion_sk = $estacion_sk and fecha_sk = $fecha_sk";
