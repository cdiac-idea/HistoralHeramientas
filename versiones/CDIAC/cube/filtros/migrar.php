...
function migrar_inicial($estacion_sk){
	...
	$consulta = "SELECT nombre from variable where id_variable in (select id_variable from esta_var where estacion_sk='$estacion_sk')";
	...
	for ($i=0; $i < $n_contar ; $i++) { 
			
		### consulta para optener la i-esima fina en central ####	

		$fila = "SELECT * from central limit 1 offset $i";
		$e_fila = pg_query($fila);
		$f_fila = pg_fetch_array($e_fila);

		### asignacion de las llaves primarias ###

		$estacion_sk = $f_fila['estacion_sk'];
		$tiempo_sk = $f_fila['tiempo_sk'];
		$fecha_sk = $f_fila['fecha_sk'];

		

		$existe = "SELECT * from fact_table where estacion_sk = $estacion_sk and tiempo_sk = $tiempo_sk and fecha_sk = $fecha_sk";
		$e_existe = pg_query($existe);
		$f_existe = pg_fetch_array($e_existe);

		if ($f_existe == false) {
		
			### insertar llaver primarias en fact_table ###

			$insert = "INSERT INTO fact_table (estacion_sk,fecha_sk,tiempo_sk) values ($estacion_sk,$fecha_sk,$tiempo_sk)";
			...
			while($f_columas_central = pg_fetch_array($e_columas_central)){
				
				### cateo de datos pues se recuperan como strin y se necesita numerico ###

				$variable=(String)$f_columas_central['column_name'];
				
				### funcion para subir el campo recuperado a la fact_table ###

				update_especific($variable,$estacion_sk,$fecha_sk,$tiempo_sk,$i);
