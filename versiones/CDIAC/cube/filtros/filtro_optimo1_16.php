...
# EXCLUSIVO PARA LA TOMA DE DATOS DEL SERVIDOR
...
$valor_maximo = "SELECT registro,estacion_sk from central"; # se consulta el total de registros de la tabla central
  ...
  $estacion_sk_a_central = "UPDATE central set estacion_sk = $estacion_sk";
  $ejecuto_agregamos = pg_query($estacion_sk_a_central);
  }

  ########## SE INSERTA FECHA_SK ##################################################

  $cantidad_fechas_distintas = "SELECT DISTINCT fecha FROM central order by fecha";
  $e_cant_fech_dist = pg_query($cantidad_fechas_distintas);
    ...
    while($a_cant_fech_dist = pg_fetch_array($e_cant_fech_dist)){
      ...
      if($fecha_sk == 0 or $fecha_sk == null){
          $fecha_sk = obtener_fecha_sk_arch($c_fecha_pos);
...
//CAST($nombre_variable AS float) < $minimo or CAST($nombre_variable AS float) > $maximo or 
  ...
  elseif ($maximo != null) {
    ...
    $errores = "SELECT registro, $nombre_variable from central where $nombre_variable = '-' or $nombre_variable is null or $nombre_variable = null";
    $e_errores = pg_query($errores);
...
function correccionSinDiferencia($registro, $datos_erroneos, $tipo_correccion, $variable){
  ...
  if($tipo_correccion == 'promedio'){
    ...
    if($dato_anterior[$variable] != '-' and $dato_anterior[$variable] != '' and $dato_anterior[$variable] != ' ' and $dato_anterior[$variable] != null and $dato_siguiente[$variable] != '-' and $dato_siguiente[$variable] != '' and $dato_siguiente[$variable] != ' ' and $dato_siguiente[$variable] != null){
  ...
  elseif($tipo_correccion == 'diferencia_de_0,2'){
      ...
      if($dato_anterior[$variable] != null and $dato_anterior[$variable] != '' and $dato_anterior[$variable] != '-' and $dato_anterior[$variable] != ' ' and $dato_Siguiente[$variable] != null and $dato_Siguiente[$variable] != '' and $dato_Siguiente[$variable] != ' ' and $dato_Siguiente[$variable] != '-'){
