    ...
    if ($_POST) {
		...
		if ($idVariable == 175) {
			$titulo1 .= "<td>Concentración<br>(ppm)</td><td>ICA CO</td><td>Clasificación</td>
							<td>Confianza (%)</td><td>Desviación <br>Estandar</td>"; //<td>Gráfica</td>";
		...
		if ($idVariable == 176) {
			$titulo1 .= "<td>Concentración<br>(ppm)</td><td>ICA O3</td><td>Clasificación</td><td>Confianza (%)</td>
						<td>Desviación <br>Estandar</td>"; //<td>Gráfica</td>";
		...
		$titulo1 .= "</tr>";
		$sql .= " FROM indicador_aire i WHERE i.estacion=".$idEstacion." AND i.anio>=".$menor." AND i.anio<=".$mayor;
		if ($idVariable == 173) {
	...
	if ($result != null) {
		...
		foreach ($result as $value) {
			...
			foreach ($value as $field) {
				...
				}elseif($field=="Dañina a la salud para grupos sensibles") {
